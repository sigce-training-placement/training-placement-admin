...
const ShowAllDrives = ({ data, setShowModal, showModal }) => {
	return (
		<>
			...
			<div className={showModal ? 'p-10 rounded-lg shadow-xl h-[90vh] w-[80vw] bg-white fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[3001] opacity-100 duration-500' : 'p-10 rounded-lg shadow-xl h-[90vh] w-[80vw] bg-white fixed top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[3001] pointer-events-none opacity-0 duration-500'}>
				...
				<div className='grid grid-cols-1 gap-y-2 mt-6'>
					{
						data && data.map((drive, index) => {
							...
							return <Link to={`/admin/drives/${drive.id}`} key={drive.id + index} className='shadow-lg w-full bg-gray-50 rounded-lg p-6 relative'>
