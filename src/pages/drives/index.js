...
const DriveMain = ({ getDate, setMessage }) => {
	...
	return (
		<>
			...
			<Layout title={"Drives"} className={"px-10 bg-custom-gradient"}>
				<PageHeader getDate={getDate} searchBar={true} data={driveArr} result={setResult} buttoncomponent={<Button text="Create" className={"py-2"} handler={() => { setShowDriveCreateModal(true) }} />} />
							...
							<div className='text-center grid grid-cols-1'>
								{
									driveArr && driveArr.map((drive) => {
										return <Link to={`/drive/${drive.id}`} key={drive.driveid} className="grid grid-cols-5 py-3 border-b">
